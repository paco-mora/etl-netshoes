...
class Netshoes_Exportacao():
    ...
    def transform(self):
        #Renomear colunas
        self.df = self.df[['SKU Netshoes', 'ID Sku', 'Categoria (Nível 2)', 'Marca', 'Nome Produto']]
        #Renomear colunas
        self.df = self.df.rename({'SKU Netshoes': 'sku_netshoes', 'ID Sku': 'sku_seller', 'Categoria (Nível 2)': 'category', 'Marca':'brand', 'Nome Produto': 'title'}, axis= 1)
        #Converter para string
        self.df['sku_seller'] = self.df['sku_seller'].astype(str)
        self.df['sku_seller'] = self.df['sku_seller'].str.lstrip('0')
...
class Template_Custo():
    ...
    def load(self):
        file = 'template_custo_' + self.user + '.xlsx'
        self.df = pd.read_excel(NETSHOES_FOLDER + file)

    def transform(self):
        self.df = self.df.rename({'Sku Seller': 'sku_seller', 'Preco De Custo': 'preco_custo'}, axis= 1)
        self.df['sku_seller'] = self.df['sku_seller'].astype(str)
        self.df['sku_seller'] = self.df['sku_seller'].str.lstrip('0')
        if(self.especial_char == False):
            self.df['sku_seller'] = self.df['sku_seller'].apply(lambda x :remove_especial_char(x))
        self.df['preco_custo'] = self.df['preco_custo'].str.replace(',', '.')
        self.df = self.df.drop_duplicates(['sku_seller'])
        self.df['sku_seller'] = self.df['sku_seller'].astype(str)
